...
<html>
	...
	<form id="mainform" name="mainform" method="post" action=""> <!-- This form will post data to an initially unspecified page when submitted. -->
		...
		<input type='hidden' name="User" value="<?php echo $_POST['User']; ?>" onload="CreateUserDataChangedEvent();"/><!-- Hidden tag used to store posted user data so that it can later be posted to other pages, then back to the home page. -->
		...
		<div class="grid-class"> <!-- Div containing main buttons that link to other pages. -->
			<input class="mainButton" type="button" id="btnNewCall" value="New Call" onClick="GoToNewPage('NewCaller?previous=Home')" />
